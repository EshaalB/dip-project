...
def generate_bias_map(L, kernel_size=5):
    # Generate bias map for local color correction (utility function)
    # Note: Model predicts its own bias map, but this can be used for post-processing
    L_norm = np.clip(L / 100.0, 0, 1)
    L_smooth = cv2.GaussianBlur(L_norm, (kernel_size, kernel_size), 0)

    bias_a = (L_smooth - 0.5) * 0.2
    bias_b = (1.0 - L_smooth) * 0.2
    bias_map = np.stack([bias_a, bias_b], axis=2)

    return bias_map.astype(np.float32)


def apply_bias_map(a, b, bias_map, strength=1.0):
    # Apply bias map correction
    a_corrected = np.clip(a + bias_map[:, :, 0] * strength, -127, 127)
    b_corrected = np.clip(b + bias_map[:, :, 1] * strength, -127, 127)
